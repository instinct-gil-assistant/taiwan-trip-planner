...
- API: Neon Function `taiwanapp`; invocation URL is configured in `index.html`.
...
- The stable datastore is the dedicated `taiwan-gil-dana` `trip_state` row containing versioned JSON. UI structure can change without dropping data.
...
4. Deploy with:
   `neon function deploy taiwanapp --project-id jolly-dawn-59684248 --branch production --src backend/index.js --runtime nodejs24 --env APP_SECRET=<vault-secret> --env STATE_ID=taiwan-gil-dana --env DATABASE_URL=<connection-string> --wait`
